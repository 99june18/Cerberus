...
#define DATA_LEN             272    // system data region (includes 16-bit CRC)
#define DATA_PAYLOAD_BITS    256    // user payload
#define SECC_CRC_BITS        16
...
// ----------------------------------------------------------------------------
static inline unsigned short crc16_ccitt_0init_over_256bits(const unsigned int data256[DATA_PAYLOAD_BITS]) {
    unsigned char bytes[DATA_PAYLOAD_BITS/8];
    for (int i=0; i<DATA_PAYLOAD_BITS/8; ++i) {
        unsigned char v = 0;
        for (int b=0; b<8; ++b) {
            v |= ((data256[i*8 + b] & 1u) << (7 - b));
        }
        bytes[i] = v;
    }
...
static inline unsigned short bits16_to_u16_msb(const unsigned int bits[SECC_CRC_BITS]) {
    unsigned short v = 0;
    for (int i=0; i<SECC_CRC_BITS; ++i) {
        v |= ((bits[i] & 1u) << (SECC_CRC_BITS - 1 - i));
    }
    return v;
}
...
int main(int argc, char* argv[]) {
    ...
    string Result_file_name = "Result/HBM4_" + L1_STR + "_" + STOR_STR + "_" + P_STR + "_" + L2_STR + ".S";
    ...
    for (int run=0; run<RUN_NUM; ++run) {
        ...
        for (int i = 0; i < 272; i++) { data_272_1[i] = data_272[i]; }
        ...
        for (int i = 0; i < 272; i++) {
            if ((data_272_1[i] != cw[i]) & (res_oecc == CE || res_oecc == NE)) {
                res_oecc = SDC;
            }
        }
        ...
        // 12) S-ECC: CRC16 detect-only over 256b
        for (int i=0; i<DATA_PAYLOAD_BITS; ++i)
            data_256[i] = msg_272_for_secc[i] & 1u;

        unsigned short crc_calc = crc16_ccitt_0init_over_256bits(data_256);

        unsigned int crc_bits[SECC_CRC_BITS];
        for (int i = 0; i < SECC_CRC_BITS; ++i)
            crc_bits[i] = msg_272_for_secc[DATA_PAYLOAD_BITS + i] & 1u;

        unsigned short crc_recv = bits16_to_u16_msb(crc_bits);

        int result_secc = (crc_calc == crc_recv) ? NE : DUE;

        if (result_secc == NE && any_one_bits(msg_272_for_secc, DATA_LEN) && (res_oecc == CE || res_oecc == NE || res_oecc == SDC))
            result_secc = SDC;
